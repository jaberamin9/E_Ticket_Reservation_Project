package com.blogspot.softlabsja.e_ticketreservationsystem;
...
public class MainActivity extends AppCompatActivity {
    ...
    String loginUrl;
    String singUpUrl;
    EditText email, pass, C_user_name, C_email, C_pass;
    Button loginBtn, singupBtn;
    TextView createNew, loginHere,changeIp;
    ContentLoadingProgressBar loadingBtn, loadingBtn2;
    AVLoadingIndicatorView loading;
    LinearLayout login_box, singup_box;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        singup_box = findViewById(R.id.singup_box);
        loginHere = findViewById(R.id.loginHere);
        C_user_name = findViewById(R.id.C_user_name);
        ...
        handler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                String email_save = prefs.getString("email", "");
                String pass_save = prefs.getString("pass", "");
                if (!email_save.equals("") && !pass_save.equals("")) {
                    login(email_save,pass_save);
                } else {
                    loading.hide();
                    ...
                    singupBtn.setTextColor(Color.parseColor("#FFFFFFFF"));
                    singupBtn.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            loadingBtn2.show();
                            singupBtn.setTextColor(Color.parseColor("#8effffff"));

                            if (C_user_name.getText().toString().trim().equalsIgnoreCase("")) {
                                Toast.makeText(getApplicationContext(), "Enter Your User Name", Toast.LENGTH_SHORT).show();
                                loadingBtn2.hide();
                                singupBtn.setTextColor(Color.parseColor("#FFFFFFFF"));
                            } else if (C_email.getText().toString().trim().equalsIgnoreCase("")) {
                                Toast.makeText(getApplicationContext(), "Enter Your Email", Toast.LENGTH_SHORT).show();
                                loadingBtn2.hide();
                                singupBtn.setTextColor(Color.parseColor("#FFFFFFFF"));
                            } else if (C_pass.getText().toString().trim().equalsIgnoreCase("")) {
                                Toast.makeText(getApplicationContext(), "Enter Your Password", Toast.LENGTH_SHORT).show();
                                loadingBtn2.hide();
                                singupBtn.setTextColor(Color.parseColor("#FFFFFFFF"));
                            } else {
                    ...
                    createNew.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View view) {
                            ...
                            singup_box.setVisibility(View.VISIBLE);
                        }
                    });
                    loginHere.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            singup_box.setVisibility(View.GONE);
                            login_box.setVisibility(View.VISIBLE);
                        }
                    });
                }
    ...
    void login(String email, String pass) {
        ...
        loginUrl = "http://"+ipAddress+"/E_TicketReservation/etiket/userLogin.php";
    ...
    void singUp() {
        SharedPreferences getIP = getSharedPreferences(IP, MODE_PRIVATE);
        String ipAddress = getIP.getString("ip", "");
        singUpUrl = "http://"+ipAddress+"/E_TicketReservation/etiket/registerUser.php";
        RequestQueue queue = Volley.newRequestQueue(this);
        StringRequest stringRequest = new StringRequest(Request.Method.POST, singUpUrl,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        try {
                            ...
                            if (jsonObject.getString("error").equals("true")) {
                                ...
                                editor = getSharedPreferences(LOGIN_DETAILS, MODE_PRIVATE).edit();
                                editor.putString("email", C_email.getText().toString().trim());
                                editor.putString("pass", C_pass.getText().toString().trim());
                                editor.apply();
            ...
            protected Map<String, String> getParams() throws AuthFailureError {
                Map<String, String> data = new HashMap<>();
                data.put("username", C_user_name.getText().toString().trim());
                data.put("email", C_email.getText().toString().trim());
                data.put("password", C_pass.getText().toString().trim());
                return data;
            }
        };
        queue.add(stringRequest);
